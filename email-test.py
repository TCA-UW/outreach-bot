...
def scan_site_for_emails(base_url):
    ...
    all_emails = set()
    print(f"\n:mag: Starting scan for: {base_url}\n")
    for path in COMMON_PATHS:
        full_url = urljoin(base_url, path)
        print(f":arrow_right:  Scanning {full_url} ...")
        emails = fetch_and_extract_emails(full_url)
        if emails:
            print(f"   :heavy_check_mark: Found: {emails}")
        else:
            print("   :x: No emails found.")
        all_emails.update(emails)
    print("\n:white_check_mark: Final list of unique emails found:")
